...
function readStored() {
    const raw = readJsonIfExists(prefsFile());
...
export function mergeUiPrefs(patch) {
    ...
    if (patch.tokenMetricsEnabled !== undefined)
        stored.tokenMetricsEnabled = patch.tokenMetricsEnabled;
    else if (previous?.tokenMetricsEnabled !== undefined)
