...
export function pathsEquivalent(a, b) {
    ...
    if (isWindowsStyle(a) || isWindowsStyle(b))
        return left.toLowerCase() === right.toLowerCase();
    return left === right;
}
export function listWritableRoots(content) {
...
export function isStateDirAllowlisted(content, stateDir) {
...
export function upsertWritableRoot(content, stateDir) {
    const tomlPath = toTomlPath(stateDir);
    if (isStateDirAllowlisted(content, stateDir))
        return content;
    const table = findTable(content, TABLE);
    if (!table) {
...
function findTable(content, name) {
...
function findArrayAssignment(tableBody, key) {
...
function parseTomlStringArray(src) {
    const values = [];
    const re = /"((?:\\.|[^"\\])*)"|'((?:\\.|[^'\\])*)'/g;
    let match;
...
//# sourceMappingURL=sandbox-allow.js.map