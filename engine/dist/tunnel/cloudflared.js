...
export function parseQuickTunnelUrl(line) {
    for (const match of line.matchAll(QUICK_TUNNEL_URL_RE)) {
        try {
            ...
            if (url.protocol !== "https:" || !QUICK_TUNNEL_HOST_RE.test(url.hostname))
                continue;
            if (url.hostname.toLowerCase() === "api.trycloudflare.com")
                continue;
            return url.origin;
        }
        catch {
 ...
 */
export class CloudflaredQuickTunnel {
    logger;
    binaryOverride;
    ...
    starting = null;
    cancelStart = null;
    constructor(logger = nullLogger, binaryOverride, options = {}) {
        this.logger = logger;
    ...
    startProcess(localPort) {
        ...
        if (!bin) {
            return Promise.reject(new Error("cloudflared is not installed. Install it (e.g. `brew install cloudflared`) and retry."));
        }
        return new Promise((resolve, reject) => {
            let child;
            try {
                child = this.spawnImpl(bin, ["tunnel", "--url", `http://127.0.0.1:${localPort}`, "--no-autoupdate", ...tunnelProtocolArgs()], { stdio: ["ignore", "pipe", "pipe"], windowsHide: true });
            }
            catch (error) {
                reject(error);
                return;
            ...
            const isAlive = () => this.child === child;
            const stopChild = () => {
                try {
                    child.kill("SIGTERM");
                }
            ...
            const finish = (callback, closeOutput = true) => {
                if (settled)
                    return;
                settled = true;
                if (timeout)
                ...
                if (closeOutput)
                    closeReaders();
                if (cancel && this.cancelStart === cancel)
            ...
            child.on("exit", (code) => {
                ...
                if (!settled) {
                    fail(new Error(`cloudflared exited (code ${code}) before establishing a tunnel${this.lastError ? `: ${this.lastError}` : ""}`));
                }
            });
        });
    }
