...
export function gitStatus(target) {
    ...
    if (!result.ok)
        return empty;
    const out = { ...empty, hidden: { ...empty.hidden }, isRepo: true };
    const withheld = (paths) => paths.some((p) => ignoreRules.isSensitive(p));
    for (const line of result.stdout.split("\n")) {
        if (line.startsWith("# branch.head ")) {
            ...
            if (xy[0] !== ".")
                out.staged.push({ path: filePath, change: xy[0] });
            if (xy[1] !== ".")
                out.unstaged.push({ path: filePath, change: xy[1] });
        }
        else if (line.startsWith("? ")) {
            const filePath = line.slice(2);
            if (withheld([filePath]))
                out.hidden.changes += 1;
            else
...
function chunkSafePaths(paths, maxCount = 50, maxBytes = 32 * 1024) {
    const batches = [];
    let currentBatch = [];
    let currentBytes = 0;
...
export function gitDiff(target, opts = {}, relPath) {
    const root = typeof target === "string" ? target : target.root;
    const ignoreRules = typeof target === "object" && target.ignoreRules
        ? target.ignoreRules
