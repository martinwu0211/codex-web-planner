...
function usageDisplay(usage, enabled) {
...
function readCappedUtf8(filePath, maxBytes) {
...
function tunnelChoicePayload(workspace, zoneHint) {
...
function trySandboxAllow() {
    try {
        ...
        return { ok: false, added: false, alreadyAllowed: false, error: error.message };
    }
}
async function ensureBridgeAndTunnel(workspaceRoot, opts) {
...
function acceptUnusedWorkspaceOption(command) {
...
program
    ...
    while (Date.now() - started <= timeoutMs) {
        ...
        if (observation.state === "healthy") {
            const info = await adminFetch(observation.runtime, "GET", "/admin/info");
...
program
    ...
    else if (chatgptConnection.lastMcpExchangeAt)
        say(`· ChatGPT 通信：上次响应 ${chatgptConnection.lastMcpExchangeAt}`);
    else
        say("· ChatGPT 通信：尚未检测到；运行 `c2c setup` 完成连接");
...
program
    ...
    if (opts.fix) {
        ...
        if (sandbox.ok) {
            ...
            if (sandbox.added)
                results.push("已将本地设置目录加入 Codex 沙箱白名单");
        }
        else {
        ...
        try {
            ...
            const allowed = fs.existsSync(configPath) && isStateDirAllowlisted(fs.readFileSync(configPath, "utf8"), getStateDir());
            report.sandbox = allowed ? { ok: true, detail: "已在白名单" } : { ok: false, detail: "未在白名单" };
        }
        catch (error) {
            report.sandbox = { ok: false, detail: error.message };
        }
    }
    // Workspace
    let workspace = null;
    try {
        ...
        report.workspace = { ok: false, detail: error.message };
    }
    // Bridge
    let runtime = null;
    ...
    if (workspace) {
        ...
        if (observation.state === "healthy") {
            ...
            try {
                ...
                report.bridge = { ok: false, detail: error.message };
            }
        }
        if (runtime)
            report.bridge = { ok: true, detail: `端口 ${runtime.port}` };
        else
            report.bridge = report.bridge ?? { ok: false, detail: "未运行" };
    }
    ...
    if (runtime) {
        try {
            ...
            report.mcp = { ok: false, detail: error.message };
        }
    }
        ...
        if ((!currentUrl || !healthy) && opts.fix && (expectedPublic || info.tunnel.running)) {
            try {
                ...
                if (!binaries.cloudflared) {
                    ...
                    const started = await adminFetch(runtime, "POST", "/admin/tunnel/start", 90_000);
                    if (started.url) {
                        const previousUrl = lastEndpoint?.publicUrl;
                        currentUrl = started.url;
                        healthy = true;
                        info = await adminFetch(runtime, "GET", "/admin/info");
                        const sameAddress = previousUrl && normalizePublicUrl(previousUrl) === normalizePublicUrl(started.url);
    ...
    if (chatgptRepair.needed && chatgptRepair.userMessage) {
        ...
        if (chatgptRepair.mcpUrl)
            say(`新的连接地址：${chatgptRepair.mcpUrl}`);
        if (chatgptRepair.pairingCode)
            say(`配对码：${chatgptRepair.pairingCode}`);
        say("");
    }
    say(allOk && !chatgptRepair.needed && !namedRepair.needed
...
acceptUnusedWorkspaceOption(program
    ...
    if (result.alreadyAllowed)
        check("沙箱白名单已就绪，后续对话无需再提权");
    else
        check("已将本地设置目录加入 Codex 沙箱白名单（后续对话无需再提权）");
});
...
acceptUnusedWorkspaceOption(program
    ...
    const emit = (data) => {
        if (opts.json)
            say(JSON.stringify({ ok: true, version: VERSION, ...data }));
...
session
    ...
    if (opts.json)
        say(JSON.stringify({ ok: true, session: saved, conversation }));
    else if (!saved) {
        ...
        if (conversation.projectUrl)
            say(`合集：${conversation.projectUrl}`);
        if (saved.title)
            say(`会话：${saved.title}`);
        if (saved.url)
            say(`对话：${saved.url}`);
        if (saved.connectorName)
            say(`连接器：${saved.connectorName}`);
        if (saved.taskId)
            say(`任务：${saved.taskId}（第 ${saved.iteration ?? 0} 轮，${saved.lastState ?? "?"}）`);
        if (saved.checkpoint) {
            say(`存档：${saved.checkpoint.protocolState} / 等待 ${saved.checkpoint.waitingFor}（第 ${saved.checkpoint.iteration} 轮）`);
        }
    }
});
session
    ...
    if (protocolRaw && !PROTOCOL_STATES.includes(protocolRaw)) {
    ...
    const waitingNorm = waitingRaw
        ...
        : undefined;
    if (waitingNorm && !WAITING_FOR.includes(waitingNorm)) {
    ...
    const saved = mergeSession(readSession(workspace.id), {
        ...
        conversationMode: modeRaw,
        projectUrl: opts.projectUrl,
        connectorName: opts.connectorName,
        clearCheckpoint: opts.clearCheckpoint,
        checkpoint: protocolRaw
            ? {
                protocolState: protocolRaw,
                waitingFor: waitingNorm ?? undefined,
...
session
    ...
    if (!result.cleared)
        say("尚未记录 ChatGPT 会话。");
    else if (result.keptProject)
        check("已清除当前对话，合集绑定仍保留");
    else
...
acceptUnusedWorkspaceOption(prefsCmd
    ...
    if (prefs.setupMode === "auto")
        say("配置方式：AI 自动化配置（预览版）");
    else if (prefs.setupMode === "manual")
        say("配置方式：手动教学配置");
    else
        say("配置方式：尚未选择");
...
program
    ...
    const rawOutput = opts.outputFile !== undefined
    ...
    if (outputId !== undefined && !outputAvailable)
        check("已记录执行摘要（输出未对 ChatGPT 开放）");
...
for (const [name, action] of [["status", "status"], ["start", "start"], ["stop", "stop"]]) {
    browserCmd.command(name).option("--port <port>", "Chrome DevTools port", "9222").option("--json", "machine-readable output", false)
        ...
        try {
            ...
            if (opts.json)
                say(JSON.stringify({ ok: result.state !== "missing", ...result }));
            else {
...
tunnelCmd
    ...
    try {
        ...
        if (payload.needsChoice)
            say(TUNNEL_CHOICE_PROMPT);
        else if (payload.namedReady)
            check(`固定域名：${payload.hostname}`);
        else
            say("当前使用临时地址。");
    }
    catch (error) {
...
acceptUnusedWorkspaceOption(tunnelCmd
    .command("login")
