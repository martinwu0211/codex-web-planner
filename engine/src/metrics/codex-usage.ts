...
function sessionFile(sessionId: string): string | null {
  ...
  const walk = (dir: string): void => {
    ...
    for (const entry of entries) {
      ...
      else if (entry.isFile() && entry.name.endsWith(`-${sessionId}.jsonl`)) matches.push(full);
    }
  };
  walk(sessions);
...
export function readCodexUsage(): CodexUsageView {
  const sessionId = process.env.CODEX_THREAD_ID ?? null;
  if (!sessionId) return { available: false, sessionId: null, inputTokens: null, outputTokens: null, totalTokens: null, fiveHourRemaining: null, weeklyRemaining: null, reason: "CODEX_THREAD_ID 未设置" };
  ...
  let limits: JsonRecord | null = null;
  for (const line of lines) {
    let event: JsonRecord;
    try { event = JSON.parse(line) as JsonRecord; } catch { continue; }
    const payload = asRecord(event.payload);
    if (!payload) continue;
    if (!usage && event.type === "token_usage_record") usage = asRecord(payload.usage) ?? asRecord(payload.thread_token_usage);
    if (!limits) {
      const rate = asRecord(payload.rate_limits);
      if (rate) limits = rate;
    }
    if (usage && limits) break;
  }
  const primary = asRecord(limits?.primary);
  const secondary = asRecord(limits?.secondary);
  const byMinutes = new Map<number, JsonRecord>();
  if (primary) { const n = number(primary.window_minutes); if (n !== null) byMinutes.set(n, primary); }
  if (secondary) { const n = number(secondary.window_minutes); if (n !== null) byMinutes.set(n, secondary); }
