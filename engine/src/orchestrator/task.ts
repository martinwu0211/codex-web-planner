...
export async function startPlanning(goal: string, timeoutMs = 120_000, mode: PlannerTask["mode"] = readUiPrefs().workMode, onWait?: (elapsedMs: number) => void): Promise<{ task: PlannerTask; ok: boolean; code: string; message: string }> {
  if (readUiPrefs().workMode === "codex") mode = "codex";
