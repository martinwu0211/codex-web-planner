...
export interface PlannerTask { taskId: string; goal: string; phase: TaskPhase; mode: "auto" | "chat" | "codex"; fallbackReason?: string; plan?: string; steps?: PlanStep[]; risks?: string[]; verification?: string[]; review?: string; updatedAt: string; }
...
export async function startPlanning(goal: string, timeoutMs = 120_000, mode: PlannerTask["mode"] = "auto", onWait?: (elapsedMs: number) => void): Promise<{ task: PlannerTask; ok: boolean; code: string; message: string }> {
  ...
  let result: Awaited<ReturnType<typeof askChatGpt>>;
  try {
    await startBrowser();
...
export async function reviewExecution(resultText: string, timeoutMs = 120_000): Promise<{ task: PlannerTask | null; ok: boolean; code: string; message: string }> {
  ...
  if (reviewing.mode === "codex") return { task: saveTask({ ...reviewing, phase: "done", review: "Skipped: native Codex mode", updatedAt: new Date().toISOString() }), ok: true, code: "CODEX_NATIVE_REVIEW_SKIPPED", message: "Native Codex mode completed without ChatGPT review." };
  ...
  if (!result.ok && reviewing.mode === "auto") return { task: saveTask({ ...reviewing, phase: "done", fallbackReason: `${result.code}: ${result.message}`, review: "ChatGPT review unavailable; accepted native Codex completion.", updatedAt: new Date().toISOString() }), ok: true, code: "REVIEW_FALLBACK_CODEX", message: "ChatGPT review unavailable; task completed in native Codex fallback mode." };
