...
plannerCmd.command("start").requiredOption("--goal <goal>", "task goal").option("--timeout <seconds>", "ChatGPT timeout", "120").option("--json", "machine-readable output", false)
  .action(async (opts: { goal: string; timeout: string; json: boolean }) => { try { const result = await startPlanning(opts.goal, Number(opts.timeout) * 1000); if (opts.json) say(JSON.stringify(result)); else { say(`${result.ok ? "✓" : "✗"} ${result.code}: ${result.message}`); say(JSON.stringify(result.task, null, 2)); } } catch (error) { handleCliError(error, opts.json); } });
