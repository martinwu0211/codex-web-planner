...
import { mergeUiPrefs, readUiPrefs, SETUP_MODES, WORK_MODES, type SetupMode, type WorkMode } from "../config/ui-prefs.js";
import {
...
function openChatGptSettings(): void {
...
acceptUnusedWorkspaceOption(
  ...
  .action((opts: { developerMode: boolean; setupMode?: string; workMode?: string; tokenMetrics?: string; json: boolean }) => {
    try {
      ...
      if (workModeRaw && !WORK_MODES.includes(workModeRaw as WorkMode)) throw new Error(`work-mode must be one of ${WORK_MODES.join(", ")}`);
      ...
      const prefs = mergeUiPrefs({
        ...
        workMode: workModeRaw as WorkMode | undefined,
      ...
      if (workModeRaw) check(`工作模式：${workModeRaw}`);
...
browserCmd.command("ask").requiredOption("--text <text>", "prompt to send to ChatGPT").option("--port <port>", "Chrome DevTools port", "9222").option("--timeout <seconds>", "response timeout", "120").option("--json", "machine-readable output", false)
  .action(async (opts: { text: string; port: string; timeout: string; json: boolean }) => {
    try {
      ...
      if (opts.json) say(JSON.stringify(result)); else { say(`${result.ok ? "✓" : "✗"} ${result.code}: ${result.message}`); if (result.response) say(result.response); }
    } catch (error) { handleCliError(error, opts.json); }
  });
...
plannerCmd.command("start").requiredOption("--goal <goal>", "task goal").option("--mode <mode>", "auto, chat, or codex", "auto").option("--timeout <seconds>", "ChatGPT timeout", "120").option("--json", "machine-readable output", false)
  .action(async (opts: { goal: string; mode: string; timeout: string; json: boolean }) => { try { if (!["auto", "chat", "codex"].includes(opts.mode)) throw new Error("mode must be auto, chat, or codex"); const result = await startPlanning(opts.goal, Number(opts.timeout) * 1000, opts.mode as "auto" | "chat" | "codex"); if (opts.json) say(JSON.stringify(result)); else { say(`${result.ok ? "✓" : "✗"} ${result.code}: ${result.message}`); say(JSON.stringify(result.task, null, 2)); } } catch (error) { handleCliError(error, opts.json); } });
