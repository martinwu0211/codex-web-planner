...
1. On first use, run the bundled setup flow from this installed plugin. Do not require a global npm install, PATH change, or a manually installed `c2c`: resolve the plugin's bundled `engine/bin/c2c.js` and invoke it with Node when the `c2c` command is unavailable. The bundled launcher automatically installs its production dependencies into the plugin cache if they are missing. Do not merely say that setup started: run the bundled CLI's `status --json` afterward and report `chatgptConnection.authorized`. If it is false, immediately execute the bundled CLI's `setup` (prefer the secure tunnel/default setup; use local-only mode only when the user explicitly requests it). Include the returned connection address and pairing code in the response with the three shortest authorization steps. Do not finish with only “未连接” or ask the user to discover the command themselves. After the user completes authorization, rerun `status --json` and report the result. If it is true but `recentMcpExchange` is false, report that authorization exists but no recent ChatGPT request has been observed, then ask the user to open or refresh the connected ChatGPT conversation and rerun the check.
