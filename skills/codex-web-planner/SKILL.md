...
3. If authorization is false, immediately execute the bundled CLI's `setup` (prefer the secure tunnel/default setup; use local-only mode only when the user explicitly requests it). Include the returned connection address and pairing code with the three shortest authorization steps, then invoke the bundled CLI's `wait-auth --timeout-seconds 300` (five-minute default; never wait longer than twenty minutes). This polls for the user's login and returns when authorization is detected; do not finish with only “未连接” or ask the user to discover the command themselves. After it returns, rerun `status --json` and continue the remaining onboarding steps automatically.
